...
# ====================================================
# HAX 纯 IPv6 专属：Sing-box + WARP 原生直连一键起飞脚本
# 特性: 自动修复 NAT64, 自动申请 WARP 密钥, 极简原生架构
# ====================================================
...
echo -e "${GREEN}=== 启动 HAX 纯 IPv6 专属极客部署 ===${RESET}"

# 1. 固化 NAT64/DNS64 (防止重启后丢失 IPv4 访问能力)
echo -e "\n${GREEN}[1/4] 正在配置并固化 NAT64/DNS64 网关...${RESET}"
echo -e "nameserver 2a00:1098:2b::1\nnameserver 2a01:4f8:c2c:123f::1" > /etc/resolv.conf
# 锁定 resolv.conf 防止被系统网络管理器覆盖
...
# 3. 下载 WARP-GO 并申请原生 WireGuard 参数
echo -e "\n${GREEN}[2/4] 正在向 Cloudflare 申请 WARP 专属私钥与暗号...${RESET}"
...
wget -qO warp-go.tar.gz "https://github.com/fscarmen/warp/releases/download/v1.0.8/warp-go_1.0.8_linux_${W_ARCH}.tar.gz"
...
if [ ! -f "warp.conf" ]; then
    echo -e "${RED}WARP 账号申请失败，请稍后重试。${RESET}"
...
# 精准提取 Reserved 数组，如果为空则默认为 [0,0,0]
WARP_RESERVED=$(grep 'Reserved' warp.conf | awk -F '=' '{print $2}' | tr -d ' ')
WARP_RESERVED=${WARP_RESERVED:-"[0,0,0]"}

echo -e "-> 提取私钥成功: ${YELLOW}隐藏显示${RESET}"
echo -e "-> 提取 IPv6 成功: ${YELLOW}${WARP_IPV6}${RESET}"
echo -e "-> 提取 Reserved 成功: ${YELLOW}${WARP_RESERVED}${RESET}"

# 4. 安装官方原版 Sing-box
echo -e "\n${GREEN}[3/4] 正在安装 Sing-box 官方最新版...${RESET}"
...
# 5. 注入灵魂：Native WireGuard 直连配置文件
echo -e "\n${GREEN}[4/4] 正在生成原生直连配置并清理战场...${RESET}"
